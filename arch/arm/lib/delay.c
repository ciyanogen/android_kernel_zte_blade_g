...
/*We need enable this function here.*/
...
void __const_udelay_zte(unsigned long xloops)
{
	unsigned long lpj;
	unsigned long loops;
	...
	//lpj = loops_per_jiffy >> 10;	/* max = 0x0001ffff */
	/*Here we using the current cpu's loops_per_jiffy instead of loops_per_jiffy
	Becasue we find this global sometimes don't update when cpu freq changed.
	*/
	lpj = 192454 >> 10;
	...
	if (loops)
		read_current_timer_delay_loop(loops);
	
}
EXPORT_SYMBOL(__const_udelay_zte);
