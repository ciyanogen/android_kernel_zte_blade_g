...
extern void manage_max8903_charging(int work_type);
...
void hsusb_chg_connected(enum chg_type chgtype)
{
	...
	printk("PM_DEBUG_MXP:Enter hsusb_chg_connected.\n");
	printk("PM_DEBUG_MXP:Charger Type: %s.\n", chg_types[chgtype]);
	if (chgtype == USB_CHG_TYPE__WALLCHARGER) 
	{
		printk("PM_DEBUG_MXP:It is a wall charger.\n");
		manage_max8903_charging(USB_CHG_TYPE__WALLCHARGER);
	}
	else if(chgtype == USB_CHG_TYPE__INVALID)
	{
		err_flag = pmic_get_chg_real_status(&chg_status);
		pr_debug("PM_DEBUG_MXP:err_flag =%d.\n", err_flag);
		pr_debug("PM_DEBUG_MXP:chg_status =%d.\n", chg_status);
		if(chg_status)
		{
			printk("PM_DEBUG_MXP:It is a unstanded charger.\n");
			manage_max8903_charging(USB_CHG_TYPE__SDP);
		}
		else
		{
			printk("PM_DEBUG_MXP:The charger has been removed.\n");
			manage_max8903_charging(USB_CHG_TYPE__INVALID);
		}
	}
	else 
	{
		printk("PM_DEBUG_MXP:It is a usb cable or carkit.\n");
		manage_max8903_charging(USB_CHG_TYPE__SDP);	
	}
	printk("PM_DEBUG_MXP:Exit hsusb_chg_connected.\n");
}
#else
void hsusb_chg_connected(enum chg_type chgtype)
{
