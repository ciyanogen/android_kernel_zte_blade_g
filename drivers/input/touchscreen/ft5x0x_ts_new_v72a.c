	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    gpio_tlmm_config(GPIO_CFG(GPIO_TOUCH_EN_OUT, 0, GPIO_CFG_OUTPUT,
...
/**************************************************/
   ret =gpio_request(GPIO_TOUCH_EN_OUT,"ts_power");
   if (ret) {
		pr_err("%s: unable to request reset gpio for TSC!\n", __func__);
		return ret;
	}
	ret = gpio_tlmm_config(GPIO_CFG(GPIO_TOUCH_EN_OUT, 0,
	...
	ret = gpio_direction_output(GPIO_TOUCH_EN_OUT, 0);	
	if (ret) 
	{		
		pr_err("%s: unable to set_direction for gpio %d\n",	__func__, GPIO_TOUCH_EN_OUT);
	}
    gpio_set_value(GPIO_TOUCH_EN_OUT, 1);
	msleep(5);
	ret = gpio_request(GPIO_TOUCH_RST_OUT, "focaltech_ts_rst");
	if (ret)
	{	
		printk("%s: gpio %d request is error!\n", __func__, GPIO_TOUCH_RST_OUT);
		return ret;
	...
	ret = gpio_tlmm_config(GPIO_CFG(GPIO_TOUCH_RST_OUT, 0,
				...
				GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (ret< 0) {
		pr_err("%s: unable to enable gpio for TSC!\n", __func__);
		return ret;
	}

	gpio_direction_output(GPIO_TOUCH_RST_OUT, 1);
	msleep(5);
	ret = gpio_request(GPIO_TOUCH_INT_WAKEUP, "focaltech_ts_irq");
	if (ret)
	{	
	...
	ret= gpio_tlmm_config(GPIO_CFG(GPIO_TOUCH_INT_WAKEUP, 0,
				GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
				GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (ret < 0) {
		pr_err("%s: unable to enable gpio for IRQ!\n", __func__);
		return ret;
	}
/*************************************************/
	{
