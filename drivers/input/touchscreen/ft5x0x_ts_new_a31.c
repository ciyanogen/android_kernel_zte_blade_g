...
static const char ts_keys_size[] = "0x01:158:94:926:108:113:0x01:172:243:926:108:113:0x01:139:393:926:108:113";
...
static void Fts_ts_work_func(struct work_struct *work)
{
	...
	else
	{
		...
		for (i = 0; i< 5; i++)
		{
			ts->finger_data[i].x = ((uint16_t)((buf[3 + i*6] & 0x0F)<<8 )| (uint16_t)buf[4 + i*6])*480/540;
			ts->finger_data[i].y = ((uint16_t)((buf[5 + i*6] & 0x0F)<<8 )| (uint16_t)buf[6 + i*6])*854/960;
