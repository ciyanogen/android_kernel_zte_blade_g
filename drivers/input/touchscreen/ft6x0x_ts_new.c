...
#if defined(CONFIG_PROJECT_P865E01) || defined(CONFIG_PROJECT_P825A20)|| defined(CONFIG_PROJECT_P865E04)
...
static const char ts_keys_size[] = "0x01:139:80:910:90:90:0x01:172:240:910:90:90:0x01:158:400:910:90:90";
...
for(ii=0;ii<4;ii++){
    ...
    Fts_i2c_write(client,0xbc,0xaa);  //FT6x06
    delay_ms(100);
     /*write 0x55 to register 0xfc*/
    Fts_i2c_write(client,0xbc,0x55); //FT6x06
    ...
    }while(reg_val[0] != 0x79 || reg_val[1] != 0x8);
}
     /*********Step 4:erase app*******************************/
...
static void Fts_ts_work_func(struct work_struct *work)
{
	...
	else
	{
		...
		//printk(KERN_ERR "%s: touch_number_haowei = %d\n", __func__,ts->touch_number);
		//printk(KERN_ERR "%s: touch_event_haowei = %d\n", __func__,ts->touch_event);
		for (i = 0; i< ts->touch_number; i++)
		{
			...
			ts->finger_data[i].event_flag = buf[3 + i*6] >> 6;
             // printk(KERN_ERR "%s: finger=%d, xx_haowei = %d\n", __func__, i, ts->finger_data[i].x);
		}
		if(ts->touch_number > 0){
		for (i = 0; i < ts->touch_number; i++)
		{
			...
			input_report_abs(ts->input_dev, ABS_MT_PRESSURE, 30);	
			input_mt_sync(ts->input_dev);
...
*/
	...
	set_bit(BTN_TOUCH, ts->input_dev->keybit);   //haoweiwei add 20130628  
	...
	refresh = create_proc_entry("driver/ts_information", 0777, NULL);
	if (refresh) {
		refresh->data		= NULL;
