...
//chengzhaoguang20130408 add diag lock
#ifdef CONFIG_ZTE_FEATURE_DIAG_LOCK_ENABLE
#define SCL_SMEM_LOG_RAM_BASE 0x2FE00000
#define ZTE_LOCK_MAGIC (0x486F7529)

unsigned char forbidded_nosubsyscmd[] =
{
    56, 57
};

unsigned char forbidded_subsyscmd[][4] =
{
    {75, 250, 31, 0},
    {75, 250, 32, 0}
};

unsigned char unforbidded_nosubsyscmd[] =
{
    //58
};

unsigned char unforbidded_subsyscmd[][4] =
{
    {75, 251, 2, 0}
};

static int get_lock_state(void);
static int init_lock_state(void);
#endif
//chengzhaoguang20130408 add diag lock end
...
//chengzhaoguang20130408 add diag lock
#ifdef CONFIG_ZTE_FEATURE_DIAG_LOCK_ENABLE
    if (ZTE_LOCK_MAGIC != get_lock_state())       // lock
    {
        int i = 0;

        if (0x4B != *temp)
        {
            if (0 == sizeof(unforbidded_nosubsyscmd))
            {
            ...
            for (i = 0; i < (sizeof(unforbidded_nosubsyscmd) / sizeof(unsigned char)); i++)
            {
                if (*temp == unforbidded_nosubsyscmd[i])
                {
                    break;
                }

                if ((i + 1) < (sizeof(unforbidded_nosubsyscmd) / sizeof(unsigned char)))
                {
                    continue;
                }
                else
        ...
        {
            if (0 == sizeof(unforbidded_subsyscmd))
            {
                driver->apps_rsp_buf[0] = 0x13;
                (void)memcpy(&(driver->apps_rsp_buf[1]), temp, len);
                ENCODE_RSP_AND_SEND(len + 1);
            ...
            for (i = 0; i < (sizeof(unforbidded_subsyscmd) / (sizeof(unsigned char) << 2)); i++)
            {
                if ((*temp == unforbidded_subsyscmd[i][0])
                    && (*(temp + 1) == unforbidded_subsyscmd[i][1])
                    && (*(temp + 2) == unforbidded_subsyscmd[i][2])
                    && (*(temp + 3) == unforbidded_subsyscmd[i][3]))
                {
                    break;
                }

                if ((i + 1) < (sizeof(unforbidded_subsyscmd) / (sizeof(unsigned char) << 2)))
                {
                    continue;
                }
                else
...
static int get_lock_state(void)
{
    void __iomem *pLockFlag = NULL;
    int retVal = 0;

    pLockFlag = ioremap(SCL_SMEM_LOG_RAM_BASE, sizeof(char));
    if (NULL == pLockFlag)
    {
        return -1;
    }

    retVal = readl(pLockFlag);
    iounmap(pLockFlag);
