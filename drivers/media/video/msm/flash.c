...
static int32_t adp1655_i2c_rxdata(unsigned short saddr,
		unsigned char *rxdata, int length)
{
	int32_t rc = 0;
	struct i2c_msg msgs[] = {
	...
	rc = i2c_transfer(adp1655_client->adapter, msgs, 2);
	if (rc < 0)
		printk("adp1655_i2c_rxdata faild 0x%x\n", saddr);
	return rc;
}
...
int32_t adp1655_i2c_read_b_flash(uint8_t raddr, uint8_t *bdata)
{
	int32_t rc = -EFAULT;
	unsigned char buf[1];
	if (!adp1655_client)
		return  -ENOTSUPP;
		buf[0] = raddr;
	rc = adp1655_i2c_rxdata(adp1655_client->addr, buf, 1);
	if (rc < 0) {
		printk("i2c_read_b failed, addr = 0x%x, val = 0x%x!\n",
            raddr, *buf);
		return rc;
	}
		*bdata = buf[0];
...
int camera_flash_set_led_state(unsigned led_state, uint8_t curr_y)
{
	...
	case MSM_CAMERA_LED_OFF:
		...
		gpio_direction_output(ADP1655_LED_FLASH_STROBE, 0);
		}
		break;
	...
	case MSM_CAMERA_LED_LOW:
		...
		gpio_direction_output(ADP1655_LED_FLASH_STROBE, 1);
		usleep_range(2000, 3000);
		adp1655_i2c_read_b_flash(0x03,&temp_reg);
		temp_reg = ((0x07&curr_y)|(temp_reg&0xf8));
		printk("%s,------333------- low mode reg 0x3 reg is %d\n", __func__,temp_reg);
		adp1655_i2c_write_b_flash(0x03, temp_reg); //
		adp1655_i2c_write_b_flash(0x04, 0xba); //torch mode
		usleep_range(2000, 3000);
		}
		break;
	...
	case MSM_CAMERA_LED_HIGH:
		...
		#endif
		adp1655_i2c_read_b_flash(0x02,&temp_reg);
		temp_reg = (temp_reg&0xf0)|((curr_y &0xf0)>>4);
		printk("%s,------111------- hight mode reg 0x2 reg is %d\n", __func__,temp_reg);
		adp1655_i2c_write_b_flash(0x02, temp_reg); //flash mode
		adp1655_i2c_read_b_flash(0x03,&temp_reg);
		temp_reg = (temp_reg&0x0f)|((curr_y &0x0f)<<4);
		printk("%s,------222------- hight mode reg 0x3 reg is %d\n", __func__,temp_reg);
		adp1655_i2c_write_b_flash(0x03, temp_reg); //flash mode
	...
	unsigned led_state)
{
	...
	case MSM_CAMERA_LED_INIT:
		if (external->flash_id == MAM_CAMERA_EXT_LED_FLASH_SC628A) {
			...
			if (!adp1655_client) {
				pr_err("adp1655_i2c_driver add client \r\n");
				rc = i2c_add_driver(&adp1655_i2c_driver);
				if (rc < 0 || adp1655_client == NULL) {
        ...
        if (rc) {
            pr_err("Flash request led_en again ---\n");
            gpio_free(external->led_en);
        ...
        if (rc) {
            pr_err("Flash request led_flash_en again ---\n");
            gpio_free(external->led_flash_en);
...
*/
	...
	case MSM_CAMERA_LED_LOW:
		...
		if (adp1655_client)
        {
            ...
            adp1655_i2c_read_b_flash(0x03,&temp_reg);
            temp_reg = ((0x07&curr_y)|(temp_reg&0xf8));
            printk("%s,------333------- low mode reg 0x3 reg is %x\n", __func__,temp_reg);
            adp1655_i2c_write_b_flash(0x03, temp_reg); //
            adp1655_i2c_write_b_flash(0x04, 0xba); //torch mode
            usleep_range(2000, 3000);
        }
