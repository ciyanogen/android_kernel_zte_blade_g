...
#ifdef SP0A28_REG_ONLINE_DEBUG//for debug online
//static u32  cur_reg=0;
//static u8 cur_val;

static uint8_t cur_reg=0;
static uint8_t cur_val;
static struct  i2c_client *sp0a28_client;


static int sp0a28_i2c_rxdata(unsigned short saddr, unsigned char *rxdata, int length)
{
    struct i2c_msg msgs[] = {
        {
            .addr  = saddr,
    ...
    if (i2c_transfer(sp0a28_client->adapter, msgs, 2) < 0) {
        CDBG("sp0a28_i2c_rxdata failed!\n");
        return -EIO;
    }
    return 0;
}
static int32_t sp0a28_i2c_txdata(unsigned short saddr, unsigned char *txdata, int length)
{
    struct i2c_msg msg[] = {
        {
            .addr = saddr,
    ...
    if (i2c_transfer(sp0a28_client->adapter, msg, 1) < 0) {
        CDBG("sp0a28_i2c_txdata faild 0x%x\n", sp0a28_client->addr);
        return -EIO;
    }
...
static int32_t sp0a28_i2c_read(uint8_t raddr, uint8_t *rdata, int rlen)
{
    int32_t rc = 0;
    unsigned char buf[1];
    if (!rdata)
    ...
    buf[0] = raddr;
    rc = sp0a28_i2c_rxdata(sp0a28_client->addr >> 1, buf, rlen);
    if (rc < 0) {
        CDBG("sp0a28_i2c_read 0x%x failed!\n", raddr);
        return rc;
    }
    *rdata = buf[0];
    return rc;
}
static int32_t sp0a28_i2c_write(uint8_t waddr, uint8_t bdata)
{
    int32_t rc = -EFAULT;
    unsigned char buf[2];
    memset(buf, 0, sizeof(buf));
    ...
    rc = sp0a28_i2c_txdata(sp0a28_client->addr >> 1, buf, 2);
    if (rc < 0)
	...
	const struct i2c_device_id *id)
{
    ...
    #ifdef SP0A28_REG_ONLINE_DEBUG//for online debug
    sp0a28_client = client;
...
static int32_t sp0a28_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    if(info->vcm_pwd)
    {
        rc = gpio_request(info->vcm_pwd, "sp0a28_2v85");
        if (rc < 0)
        {
