...
#define BATTERY_LOW_WARNING 3600
...
static int is_under_charger_mode_flag = 0;
static int  batt_cap_resume =0;
static int  batt_cap_suspend =0;
...
static struct timespec unchg_last_late_resume;
static struct timespec unchg_current_late_resume;
//static int initial_battery_capacity;
static int charge_flag = 0; //0:initial  1:charging 2:uncharging
extern  unsigned int boot_reason_kernel;
static u32 boot_reason_kernel_real =0;
//doumingming add 20121106 --
#endif
//shiyan modify for v70 20120217 ---
...
static int  __init is_under_charger_mode(char *s)
{
        if(strcmp("charger", s) == 0)
                is_under_charger_mode_flag = 1;
        else
                is_under_charger_mode_flag = 0;
        #ifdef BATTERY_DEBUG
        printk("doumingming is_under_charger_mode_flag= %d \n", is_under_charger_mode_flag);
        #endif
        return 0;
}

__setup("androidboot.mode=", is_under_charger_mode);
...
//doumingming add 20121212 add for v72a temp protect warning --
	...
	else
	{
		...
		if(charger_wake_lock_flag == 2)
		{
		        charger_wake_lock_flag =0;
		        #ifdef BATTERY_DEBUG
       		printk("doumingming: charger_wake_lock_flag=2 time unlock.\n");	
       		#endif
      			wake_lock_timeout(&charger_wake_lock, 3*HZ);
		}
	}
	...
	//if(!read_otg_flag())
	{
		if (msm_batt_info.charger_type != charger_type) 
		{
			...
			else 
			{
				...
				{
					printk("BATT: AC Wall charger removed\n");
				}else if (msm_batt_info.current_chg_source & USB_CHG)
				{
					printk("BATT: USB charger removed\n");
				}else
					printk("BATT: No charger present\n");
...
//doumingming 20121106
	...
	ktime_get_ts(&unchg_last_late_resume);
       #ifdef BATTERY_DEBUG
...
//        if (0){
       #ifdef BATTERY_DEBUG
	printk("doumingming:enter early_suspend really\n");
	#endif
		rc = msm_batt_modify_client(msm_batt_info.batt_handle,
				msm_batt_info.voltage_fail_safe,
				BATTERY_VOLTAGE_BELOW_THIS_LEVEL,
...
//doumingming 20121106
	...
	ktime_get_ts(&unchg_current_late_resume);
	if (msm_batt_info.batt_handle != INVALID_BATT_HANDLE) {
		rc = msm_batt_modify_client(msm_batt_info.batt_handle,
		msm_batt_info.voltage_fail_safe,
		BATTERY_ALL_ACTIVITY,
...
#ifdef CONFIG_HIGH_VOLTAGE_BATTERY_2000mAh
static  const BattFuelCapacity fuelCapacity_norm[] = {
   {3450, 0},                      /*   0% real 3% ,3525 --> 5%*/ //doumingming modify from {3300，0}
...
//	static u32 chg_final_time =0;
	static int unchg_sleep_time = 0;
	static int unchg_late_resume_time = 0;
	u32 battery_volt =0;
	...
	//充电器插入
	if ((msm_batt_info.batt_status == POWER_SUPPLY_STATUS_CHARGING) || boot_reason_kernel_real)
	{
		unchg_time = 0;
		last_unchg_time =0;
		boot_reason_kernel_real = 0;
		if(charge_flag !=1)  {
		...
		#endif
                #ifdef BATTERY_DEBUG
 		       printk("doumingming:batt_has_charged_full  prepare enter %d\n", last_battery_capacity);
 		  #endif
		if((is_under_charger_mode_flag == 1) && (1 == charger_wake_lock_flag) && (wl_initialized) && (last_battery_capacity == 100))					
        		{
        		#ifdef BATTERY_DEBUG
        		printk("doumingming:batt_has_charged_full  enter\n");
        		#endif
        		wake_unlock(&charger_wake_lock);
   		      	wake_lock_timeout(&charger_wake_lock, 3*HZ);
		...
		else
		{
			...
			if((chg_time >= TIMEOUT_MIN)&&(current_voltage > 4250))
			{
				batt_cap = 100 ;
				batt_has_charged_full = 1;
			}
			else
			{
				...
				if ((batt_cap >= 100)&&(current_voltage >= 4320))
				{
					//batt_cap = 99;
				...
				else if ((batt_cap >=100) && (current_voltage < 4320))
				{
				       batt_cap =99;
				}
				#ifdef BATTERY_DEBUG
...
//		chg_final_time =0;
		...
		unchg_late_resume_time = (unchg_current_late_resume.tv_sec - unchg_last_late_resume.tv_sec)/60;
		unchg_sleep_time = (((unchg_sleep_time_current.tv_sec - unchg_sleep_time_start.tv_sec)/60) - unchg_late_resume_time);    //doumingming add 20121106
		#ifdef BATTERY_DEBUG
		...
		printk("doumingming: unchg_current_late_resume %ld\n", unchg_current_late_resume.tv_sec);
		printk("doumingming: unchg_last_late_resume %ld\n", unchg_last_late_resume.tv_sec);
		printk("doumingming:unchg_late_resume_time=2 %d\n", unchg_late_resume_time);
		#endif
		...
		if(0 == batt_has_charged_full )
		{
			...
			printk("doumingming:not chg batt_cap_resume %d\n", batt_cap_resume);
		       printk("doumingming:xxx(last_battery_capacity - (unchg_late_resume_time>>1) - (unchg_sleep_time>>5)) %d\n", ((unchg_late_resume_time>>1)+(unchg_sleep_time>>5)));
			#endif
			...
			if ((last_battery_capacity != 1000)&&(batt_cap < last_battery_capacity))
			{
				...
				if(batt_cap_resume)
				{
					//batt_cap = last_battery_capacity - 1;
					//batt_cap_resume =0;
					if((last_battery_capacity - (unchg_late_resume_time>>1) - (unchg_sleep_time>>5))  > 0)
					{
					if(batt_cap < (last_battery_capacity - (unchg_late_resume_time>>1) - (unchg_sleep_time>>5)))
					{
					batt_cap = (last_battery_capacity - (unchg_late_resume_time>>1) - (unchg_sleep_time>>5));
					}
					}
				}
				else
				{
					...
					if(batt_cap_suspend == 1)
					{
						if((last_battery_capacity - batt_cap) >15)
						batt_cap = last_battery_capacity - 3;	
						else
						batt_cap = last_battery_capacity - 1;	
					}
					else if((batt_cap_suspend != 1) && (((unchg_time%2==0)&&(unchg_time != last_unchg_time)&&(last_battery_capacity > 40))||((unchg_time != last_unchg_time)&&(last_battery_capacity <= 40))))
					{
						batt_cap = last_battery_capacity - 1;	
			...
			else if((last_battery_capacity != 1000)&&(batt_cap > last_battery_capacity))
			{
		       ...
		       #ifdef CONFIG_CAPACITY_FORCE_DOWN
			if(((last_battery_capacity  - (unchg_sleep_time>>7)) > 0) && (batt_cap_resume))
			{
			if(batt_cap >= (last_battery_capacity  - (unchg_sleep_time>>7)))
			batt_cap = (last_battery_capacity  - (unchg_sleep_time>>7));	
			printk("doumingming:down forcely\n");
			}
			#endif
			if(batt_cap_resume)
			{
	...
	#ifdef BATTERY_DEBUG
	printk("doumingming:  chg final  battery_cap = %d %% \n",  batt_cap);
	#endif
	 return batt_cap;
}
...
static int __devinit msm_batt_probe(struct platform_device *pdev)
{
	...
	boot_reason_kernel_real = (boot_reason_kernel & 0x60);
printk("doumingming: enter msm_batt_probe high voltage battery\n");
#ifndef CONFIG_BATTERY_MSM_FAKE
	...
	if (!msm_batt_info.voltage_fail_safe)
		msm_batt_info.voltage_fail_safe  = BATTERY_LOW_WARNING;
...
static int __devinit msm_batt_probe(struct platform_device *pdev)
{
	...
	boot_reason_kernel_real = (boot_reason_kernel & 0x60);
printk("doumingming: enter msm_batt_probe common battery\n");
#ifndef CONFIG_BATTERY_MSM_FAKE
	if (pdata->avail_chg_sources & AC_CHG) {
#else
	{
